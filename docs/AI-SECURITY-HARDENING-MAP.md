...
| 6 | **Supply-chain integrity** | AI Blueprint; NIST CSF (Protect) | Apache-2.0 open source; REUSE-3.3 license compliance; lockfile-integrity deps installed via `npm ci`; SLSA provenance + CycloneDX SBOM CI (see `apex/innovation-sprint1`); core CI workflows use SHA-pinned actions (bench/formal/fuzz currently tag-pinned — SHA-pinning tracked); gitleaks. | Downstream consumers should verify provenance on release artifacts. **Note:** the CI SLSA/Sigstore provenance signature is **classical** (ECDSA-P256); PQ provenance is available via the in-repo COSE_Sign1 / ML-DSA-87 supply-chain statements, not yet wired into the CI release path. |
