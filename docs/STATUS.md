...
**v:2 receipt linkage — binding PRIMITIVE now IMPLEMENTED (unaudited).** The commitment-to-intent gap (a malicious issuer could carry two unlinked amount commitments — `commitments.intent = SHA3(intent)` and a perfectly-hiding `Pedersen(C_amount)` — with nothing proving they hold the same value) is closed by **structural binding** (ADR-0013, after adversarial council review replaced a heavy ZK equality circuit): `disclosure/commitbind.ts` hashes the commitment _into_ the intent digest, so substitution is rejected (6 tests, incl. malicious-substitution + bad-opening). The committed value is derived from `intent.amount` with a safe-integer guard; `verifyBoundAmount` performs the full opening check. **UNAUDITED**; full v:2 receipt-body wiring + external ZK audit still pending.

**Top forward upgrade (Team Apex deep audit, 2026-06-20):** migrate the commitment layer from classical Pedersen/ristretto255 to a **post-quantum commitment scheme** (lattice- or hash-based), preserving the v:2 SHA3 binding + re-running C21 — this makes the ZK layer's _soundness_ post-quantum end-to-end (today the commitment _hiding_ is PQ/information-theoretic, but the proof _soundness_ is classical/transitional, discrete-log), and raises the audit bar from a bespoke classical proof to a PQ primitive + binding argument. The proof's zero-knowledge is also only ROM-proven, not QROM-analyzed. See [ASSURANCE.md](./ASSURANCE.md).
...
1. **View-change cert proves only round `r-1`, not a chain from round 0 — round-skip fairness gap
   ...
   binds only `(suite, height, prevHash, round)`). Because no cert _chains_ back to round 0, a **≥2/3
...
2. **Validator-set identity / epoch binding into consensus signatures — IMPLEMENTED 2026-06-28
   (ADR-0020/B5 as-built).** Previously the signed attestation/timeout messages bound suite, height,
   `prevHash`, and round but **no validator-set id or epoch**, so a vote/attestation made under one
   configuration could be replayed against a different set sharing the same `(height, prevHash, round,
suite)` — cross-epoch consent transfer. Now closed: `consensusSetId(set)` (SHAKE256 over the sorted
   `(pubkey, stake, vrfPubkey)` list + `set.epoch`) is folded into both `attestMessage`
   (`polarseek-attest-v2`) and `viewChangeMessage` (`polarseek-timeout-v2`); verifiers
   (`verifyFinalized`, `verifyViewChangeCert`, `verifyEquivocationProof`) recompute it from their own
...
3. **Equivocation is DETECTED but slashing is NOT enforced end-to-end (LEDGER-006).**
   ...
   build proofs, or apply a slash. So the accountable-safety _evidence_ primitive exists, but there is
