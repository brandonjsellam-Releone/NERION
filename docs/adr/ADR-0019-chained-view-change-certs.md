...
view-change certificate. It is a *design decision record*, not a security result, and **routes to
...
stateless and sound *for what it proves*: sub-1/3 stake cannot forge a cert; it is bound to
...
A round-`r` block proves **only that round `r - 1` timed out** — *one* cert — not that rounds
...
- **Safety is unaffected.** Each block still needs its own ≥2/3 attestations and accountable-safety /
  ...
  `equivocation.ts`). Round is *deliberately* omitted from `attestMessage` so all same-height
...
- **Exploitable only by a quorum that already controls liveness.** A <1/3 adversary cannot forge any
  cert, chained or not, so it cannot skip at all. The gap strictly concerns *fairness of leader
  allocation among an already-colluding ≥2/3*.

The cost of skipping `N` rounds today is **O(1)** (one cert for the target round). The intended
property is that skipping `N` rounds should cost **O(N)** — a *chain* of `N` valid certs — so that
...
Define a canonical, domain-separated digest over a cert's *quorum-relevant* content. Using the
...
- The `TimeoutVote` struct is **unchanged** (no per-vote chaining field): the chain link lives on the
  ...
  cross-height replay. (An alternative that binds the prev digest into each *vote's* signed message is
  considered and deferred in *Alternatives* — it is stronger but a larger protocol change.)

`Block.viewChangeCert` continues to carry the cert for `round - 1`; the *chain* it heads is validated
...
Promote `height` and `prevHash` onto `ViewChangeCert` (today they are verifier *parameters*, derived
...
*Informal, internal, pre-audit. Stated as an argument to be checked, not a guarantee.*
...
- **No new safety surface.** The chain governs *only which round may propose*; finality is still ≥2/3
...
- **Cert availability.** A block carries one cert object, but the verifier needs the *whole* chain.
  ...
  **fail closed** (treat the chain as invalid), or the linearity property is only enforced at the
  ...
  must weigh (see *Alternatives* for a logarithmic variant).
- **Honest threshold.** The whole mechanism only constrains a ≥2/3 coalition's *fairness*; it assumes
  the existing ≥2/3-honest-stake safety assumption. It does **not** defend against ≥2/3 that is willing
  to pay the linear cost — it makes skipping *expensive*, not *impossible*.
...
- **Conformance suite:** add a new check **C24** in `conformance/src/suite.ts` (next free id after C23)
  asserting the chained property end-to-end — "advancing N rounds requires N valid chained certs; a lone
  high-round cert is rejected; a broken link is rejected" — mirroring the C22/C23 style (self-contained,
  boolean). `runConformance` total increments **23 → 24**; the `STATUS.md` conformance count and the
  "23-of-23" references are bumped to 24-of-24 **in the implementing PR, not here**.
...
1. **Bind `prevCertDigest` into each `TimeoutVote`'s signed message (per-vote chaining) — DEFERRED,
   ...
   *attest to the chain*, not just to a round timeout, removing any reliance on a faithful aggregator to
...
4. **Bind `round` into `attestMessage` so attestations themselves pin the round — REJECTED (already
   ...
   accountable safety relies on (LEDGER-EQUIV-001). The round-skip gap must be closed in the *view-change
   cert*, not the attestation — which is exactly what this ADR does.
...
- **Fairness:** a ≥2/3 coalition can no longer cheaply skip to re-roll the VRF leader; doing so costs a
  full quorum-signed cert *per round skipped*. The leader-allocation bias of LEDGER-007 is mitigated
...
- **No regression risk while deferred:** default-off behind `requireCertChain`; the tree is unchanged by
  this ADR, so all 313 tests and 23-of-23 conformance remain green until the implementing PR lands.
- **Still UNAUDITED:** this records a design decision and a fix *plan*, not a security result. The
