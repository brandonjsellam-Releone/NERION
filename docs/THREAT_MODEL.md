...
> implementation-status note in §5 is retained as a reminder that implementation ≠ audited
> and that several open residuals remain (see §7). Deployment maturity is **Local/Private dev
> (UNAUDITED)**; four external launch gates remain (FTO, crypto/ZK audit, HSM/TEE hardware,
> FIPS CMVP).
>
...
- **Risk tiers T0–T3.** T0 = trivial/reversible; T3 = high-consequence/irreversible. Tier
  selects _how much assurance is required before/around an action_ (e.g., T3 may require Plane 2
...
**Cryptographic inventory (integrated and tested):**
...
`@noble/ciphers` provides AES. These primitives are wired into the protocol stack: ML-DSA-87
signs receipts (`receipts/`), ML-KEM-1024 is used in the hybrid KEM (`crypto/`),
...
| ADV-9 | **HNDL adversary (record now)**               | A _present-day_ passive collector storing ciphertext/transcripts for future quantum decryption                | Future confidentiality break of today's data           |
...
post-quantum — **the PermitToken MAC is _not_ the quantum-weak link.** The quantum-weak links are
...
- **TB-1 (Agent ↔ Kernel):** The kernel must treat _all_ agent input as hostile and untrusted.
...
- **TB-2 (Agent/Resource ↔ PermitToken):** The token crosses an untrusted hop. The resource
  trusts the token _only_ via MAC verification against a shared/derived key and binding checks.
...
- **TB-4 (P2 → Log):** The log operator is a _separate trust domain_ and is assumed potentially
  hostile (ADV-5). P2 must not trust the log to be honest; it must be able to _prove_
...
cross-plane verification. Key isolation is **designed into the implementation** (each plane uses
distinct key material and separate signing paths), but the end-to-end cross-plane isolation
guarantee has **not been formally verified or externally audited**. Treat this as an open
audit item, not a closed guarantee.
...
| T-P1-6  | **Kernel non-determinism (A14)**                   | Any wall-clock, RNG, map-iteration-order, float, locale, or unpinned-dependency nondeterminism makes two kernels disagree on admit/deny and makes receipts unreproducible. Determinism is a _security_ property here. |
...
| T-P2-6 | **Receipt ↔ action binding weakness**              | A receipt that doesn't cryptographically bind the _exact_ admitted action, attestation, and principal proves nothing useful.                                                                           |
...
- **M-P1-1 (replay):** Bind the token to a fresh, single-use attestation challenge and a tight
  ...
  valid for _that_ verb at _that_ resource. Statelessness forbids server-side nonce caches, so
  replay resistance must live in _binding + short lifetime_, and the design must state plainly:
...
- **M-P2-1 (gap):** Make the nearline window _bounded and measured_; for T3, require receipt
...
- A **Merkle inclusion proof + verified consistency proof** proves an entry is in a log whose
  history was not rewritten _relative to roots you have witnessed_.
...
- **Geo-location / IP geolocation** is **EVIDENCE, not proof.** It is trivially spoofable (VPN,
  ...
  It may be _recorded in receipts as a labeled signal_ and used for risk scoring/anomaly
...
1. No T-tier may be _lowered_, and no action _admitted_, on the strength of EVIDENCE alone.
