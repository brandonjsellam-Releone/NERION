...
| ZK range proof + policy‑satisfaction | Implemented · Tested · Conformance (C11/C13) | `disclosure/zkrange.ts`, `policyproof.ts` | **UNAUDITED, bespoke** construction. **Soundness is CLASSICAL** (discrete‑log) — a future quantum computer could *forge* a satisfaction proof. Zero‑knowledge proven in the **classical ROM, not QROM** |
| Amount confidentiality (Pedersen hiding) | Implemented | perfectly‑hiding commitment | **Information‑theoretic / PQ for the COMMITMENT** — a quantum adversary cannot *recover* the amount from a logged proof. This is distinct from the proof's (classical) soundness above |
| v:2 commitment‑to‑intent binding | Implemented · Tested · Conformance (C21) | `disclosure/commitbind.ts`, ADR‑0013 | **UNAUDITED**. Defends against a *substituting* issuer; does **not** defend against a kernel that is itself malicious at admission (that's the quorum/attestation trust model) |
...
| Action enforcement (a denied action never executes) | Implemented · Tested (C8) | kernel → PermitToken; MCP adapter | Enforcement is **at admission**: the actuator must honor the permit. An out‑of‑scope actuator that ignores Nerion is outside the trust boundary |
...
| 297 automated tests · 21/21 conformance | Tested · Conformance‑checked | `npm run gate`, `npm run conformance` | Demonstrates **implementation consistency against Nerion's own spec — NOT external security validation, formal proof, or coverage/fuzzing guarantees** |
