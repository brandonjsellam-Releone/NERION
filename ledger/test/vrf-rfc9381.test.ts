...
/**
 * RFC 9381 Appendix B — ECVRF-EDWARDS25519-SHA512-TAI (suite_string 0x03).
 * Transcribed from https://www.rfc-editor.org/rfc/rfc9381.txt (Examples 16-18).
 * These convert our self-consistent VRF into a STANDARDS-CONFORMANT one.
 */
const VECTORS = [
  {
...
describe('ECVRF-EDWARDS25519-SHA512-TAI — RFC 9381 Appendix B vectors', () => {
  for (const [i, v] of VECTORS.entries()) {
    it(`vector ${i} (alpha="${v.alpha}"): keygen + prove + proofToHash + verify match the RFC`, () => {
      const sk = hexToBytes(v.sk)
      const alpha = v.alpha === '' ? new Uint8Array(0) : hexToBytes(v.alpha)
      expect(bytesToHex(vrfPublicKey(sk))).toBe(v.pk)
      const { proof, beta } = prove(sk, alpha)
      expect(bytesToHex(proof)).toBe(v.pi)
      expect(bytesToHex(beta)).toBe(v.beta)
      const recovered = verify(hexToBytes(v.pk), alpha, hexToBytes(v.pi))
      expect(recovered).not.toBeNull()
      expect(bytesToHex(recovered as Uint8Array)).toBe(v.beta)
    })
  }
})
