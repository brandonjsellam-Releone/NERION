...
```bash
npm run gate          # clean-room lint (F1–F8) + prettier --check + tsc --noEmit + 365 tests
npm run conformance   # certification report → must stay 24/24 CONFORMANT
```
...
Nerion's entire technical _and legal_ thesis is that it governs **typed actions**, never **perception**.
...
2. `npm run gate` green + `npm run conformance` 24/24 locally.
